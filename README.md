...
## RPC messages
[`msgpack`](https://msgpack.org/) is the binary format used to transmit data throughout the neroshop network. JSON data is first converted to msgpack before it is trasmitted. This is because msgpack reduces the JSON data size by 32%, making it smaller, faster and more efficient to transfer data through the network.
